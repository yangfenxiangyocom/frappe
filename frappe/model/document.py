...
class Document(BaseDocument):
	...
	def load_from_db(self):
		if not getattr(self, "_metaclass", False) and self.meta.issingle:
			...
			if not d:
				frappe.throw(_("{0} {1} not found").format(_(self.doctype), _(self.name)), frappe.DoesNotExistError)
			self.update(d)
	...
	def validate_value(self, fieldname, condition, val2, doc=None, raise_exception=None):
		...
		if not frappe.compare(val1, condition, val2):
			...
			if doc.parentfield:
				msg = _("Incorrect value in row {0}: {1} must be {2} {3}").format(doc.idx, _(label), condition_str, val2)
			else:
				msg = _("Incorrect value: {0} must be {1} {2}").format(_(label), condition_str, val2)
